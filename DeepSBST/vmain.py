    ...
    def valid(self, model, poolsize, K):
        ...
        def ACC(real,predict):
            ...
            for val in real:
                ...
                # print("val" , val)
                # print("len val", len(real))
                # print("sum/float(len(real))" , sum/float(len(real)))
        ...
        for i in tqdm(range(data_len)):
            ...
            print("We only need to divide each by float(data_len)!!")
            print("acc",acc)
            print("mrr",mrr)
            print("map",map)  
            print("ndcg",ndcg)  
            print("float(data_len)" , float(data_len))                    
...
if __name__ == '__main__':
    ...
    mutated_mn, mutated_api, mutated_token = vDNNTestSuite.generateTestSuite(r, threshold_CC, threshold_MC, symbols_SQ, seq, TestCaseNum, minimalTest, TargMetri, CoverageStop, model, methnames, apiseqs, tokens)
    # #############3
    # mutated_mn, mutated_api, mutated_token = vDNNTestSuite.generateTestSuite(r, threshold_CC, threshold_MC, symbols_SQ, seq, TestCaseNum, minimalTest, TargMetri, CoverageStop, model, methnames, apiseqs, tokens)
    ...
    print("len mutated_mn", len(mutated_mn) )
    print("len mutated_api", len(mutated_api) )
    print("len mutated_token", len(mutated_token) )
    ...
    print("len mn", len(methnames) )
    print("len api", len(apiseqs) )
    ...
    #hist = model.fit([mutated_mn, mutated_api, mutated_token, good_descs, bad_descs], epochs=1, batch_size=128, validation_split=0.2)
    engine.valid(model, 2*number_of_samples , int(number_of_samples/2))
    # engine.valid(model, 2*number_of_samples , 3*number_of_samples/4)
