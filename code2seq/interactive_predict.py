...
def convert_method_to_input(method_string,config):
    path_extractor = Extractor(config, EXTRACTION_API, config.MAX_PATH_LENGTH, max_path_width=2)
    try:
        predict_lines, pc_info_dict = path_extractor.extract_paths(method_string)
        return predict_lines
    except Exception as e:
        print('error found when convert method string into input:\t',e)
        return None
