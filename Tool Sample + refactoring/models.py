...
class JointEmbeddingModel:
    ...
    def build(self):
        ...
        print("methname_embedding",methname_embedding)
        ...
        code_repr=Dense(self.model_params.get('n_hidden',400),activation='tanh',name='dense_coderepr')(methname_repr)
        
        
        self._code_repr_model=Model(inputs=[methname],outputs=[code_repr],name='code_repr_model')     
        ...
        code_repr=self._code_repr_model([methname])
        desc_repr=self._desc_repr_model([desc])
        cos_sim=Dot(axes=1, normalize=True, name='cos_sim')([code_repr, desc_repr])
        
        sim_model = Model(inputs=[methname,desc], outputs=[cos_sim],name='sim_model')   
        ...
        '''
        good_sim = sim_model([self.methname, self.desc_good])# similarity of good output
        bad_sim = sim_model([self.methname, self.desc_bad])#similarity of bad output
        ...
        self._training_model=Model(inputs=[self.methname,self.desc_good,self.desc_bad],
